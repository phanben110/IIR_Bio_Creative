...
class PrepareDataset:
    def __init__(self, **kwargs):
        ...
        self.n_sentences = 880  # Number of sentences to include in the dataset
        self.train_split = 0.8  # Ratio of the training data split
        self.val_split = 0.2  # Ratio of the validation data split
    ...
    def encode_pad(self, dataset, tokenizer, seq_length, who='none'):
    ...
    def __call__(self, filename, **kwargs):
        ...
        trainX = self.encode_pad(train[:, 0], enc_tokenizer, enc_seq_length, 'trainX')
        # print(f'prepare dataset - shape of trainX: \n {shape(trainX)}')
        trainY = self.encode_pad(train[:, 1], dec_tokenizer, dec_seq_length, 'trainY')

        # Encode and pad the validation input
        valX = self.encode_pad(val[:, 0], enc_tokenizer, enc_seq_length, 'valX')
        valY = self.encode_pad(val[:, 1], dec_tokenizer, dec_seq_length, 'valY')
